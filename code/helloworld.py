...
def hello(
    ...
    with open(outputfile, "at") as f:
        for name in names:
            message = "(step: {0}) - Hello {1}!\n".format(stepname, name)
            for char in message:
                f.write("{}".format(char))
                f.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--stepname", help="Step name.", default="stepname", required=False,
    )
    ...
    logging.info(
        "Parameters: inputfile={0} outputfile={1} sleeptime={2} stepname={3}".format(
            parsed_args.inputfile,
            parsed_args.outputfile,
            parsed_args.sleeptime,
            parsed_args.stepname,
        )
    )
